...
from models. gcn_inter import GCSNetInteraction
...
summary_file = 'data/result/GCNNetShareInter/GCNNetShareInter_best_per_fold_summary.txt'
...
for i in range(5):
    ...
    #保存模型
    model_file_name = 'data/result/GCNNetShareInter/GCNNetShareInter(DrugA_DrugB)' + str(i) + '--model_' + datafile +  '.model'#每一折训练好的模型
    result_file_name = 'data/result/GCNNetShareInter/GCNNetShareInter(DrugA_DrugB)' + str(i) + '--result_' + datafile +  '.csv'#每一折的预测结果
    file_AUCs = 'data/result/GCNNetShareInter/GCNNetShareInter(DrugA_DrugB)' + str(i) + '--AUCs--' + datafile + '.txt'#每轮训练的评估指标
